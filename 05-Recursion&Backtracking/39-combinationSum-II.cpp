...
public:
    void combinationHelper(vector<int>& candidates, int target, vector<int>& output, vector<vector<int>>& ans, int index){
        ...
        for(int i=index; i<candidates.size(); i++){
            ...
            combinationHelper(candidates, target-candidates[i], output, ans, i+1);  // 1) i send as it is, so that can use multiple times a single value
            output.pop_back();
        }
    }
...
public:
    void combinationHelper(vector<int>& candidates, int target, vector<int>& output, vector<vector<int>>& ans, int index){
        ...
        for(int i=index; i<candidates.size(); i++){
            output.push_back(candidates[i]);
            combinationHelper(candidates, target-candidates[i], output, ans, i+1);  // i send as it is, so that can use multiple times a single value
            output.pop_back();
        }
    }
